...
## 데이터셋 (수동 준비 — 공유 데이터, read-only)

DUT-Anti-UAV는 리포에 **커밋하지 않습니다**. 아래 PASCAL VOC 구조로 `/mnt/ssd_0/dataset/DUT`에
배치/압축해제 하세요 (변환 스크립트는 이 트리를 **수정하지 않습니다**):
...
**imgsz=960 과 P2 head를 1차 정확도 레버로 권장**합니다.
...
공유 데이터셋을 호스트 경로 → 동일한 컨테이너 경로로 마운트하므로 `configs/dut_drone.yaml`이
네이티브/컨테이너 양쪽에서 그대로 동작합니다. 다른 머신에서는 `docker-compose.yml`의 데이터셋
볼륨과 config의 `path:` 한 줄만 바꾸면 됩니다.
...
약 4배(5.2→20.5)입니다. ML2 CPU 타깃에는 **yolo26n을 배포 모델로 권장**하며, 지연 예산에 여유가
있으면 yolo26s가 정확도 상한선입니다. 예측 예시(매우 작은 드론, conf 0.78): `docs/demo/`.
...
비교군 **yolo26s**도 동일 경로로 export 했습니다: FP32 38.2 MB / FP16 19.2 MB / INT8 10.2 MB
...
> ML2 Zen2 모바일 CPU의 *방향성* 추정치이며 실제 측정값이 **아닙니다**. 최종 수치는 ML2
> on-device ADB 프로파일링이 필요합니다 (디바이스는 ORT + XNNPACK 사용).
...
INT8은 크기와 단일 스레드 지연에서 유리합니다. 4 스레드에서는 Conv-only QDQ의 dequant 오버헤드로
x86에서 격차가 좁아집니다(ML2의 XNNPACK은 동작이 다름). FP16은 CPU 속도 이득이 없으며(ORT CPU에
native fp16 커널 없음) 크기/이식성 옵션입니다.
...
CPU Execution Provider + XNNPACK.** CPU 추론으로 RDNA2 iGPU를 120Hz AR 스테레오 렌더링에 비워둡니다.
...
좌표계 기준이며, letterbox를 역산(패딩 빼고 스케일로 나눔)해 카메라 프레임 좌표로 매핑합니다.
...
재 export. 지연 예산에 여유가 있으면 yolo26s가 정확도 상위 대안입니다.

**한계 / 정직한 고지:** 본 리포의 지연 수치는 x86-64 데스크톱 CPU 측정값으로 ML2 Zen2 모바일
CPU의 **방향성 추정치**일 뿐 실측이 아닙니다. 실제 on-device 지연/정확도는 ML2 ADB 프로파일링으로
확인하세요. INT8은 크기·단일 스레드 지연에서 가장 유리하며, 멀티 스레드 이득은 디바이스 XNNPACK
커널에 따라 달라집니다.

**선택적 대안:** TFLite INT8 (`format='tflite', int8=True`) — ML2의 TFLite + NNAPI/XNNPACK
경로용. TensorFlow/onnx2tf 툴체인 필요.

---

## 라이선스 / 비고

데이터셋(DUT-Anti-UAV)은 자체 라이선스를 따르며 여기서 재배포하지 않습니다.
