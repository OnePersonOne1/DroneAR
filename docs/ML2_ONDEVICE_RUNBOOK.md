...
ML2 실기 보유자가 따라 실행하는 체크리스트. **이 리포 작업에서는 실행하지 않았다**(기기 없음).
수치 칸은 비워 둔다 — 측정 후 채운다. ⚠️ 측정 전 값을 추정·기입하지 말 것.
...
호스트(RTX 4090 Vulkan)에서 추론 정확성 + Vulkan 경로는 이미 검증됨(parity PASS).
ML2 고유 미지수는 두 가지뿐: **(a) AOSP 빌드가 앱에 Vulkan compute queue 를 노출하는가, (b) RDNA2 실측 성능.**
...
> 비교 표의 CPU 7 FPS 는 ML2 팀이 제공한 현재 측정치다. GPU 값은 측정 후 기입.
> 추론 디바이스(CPU/GPU)는 ML2 팀 결정 사항이며, 본 표는 선택을 돕는 자료다.
